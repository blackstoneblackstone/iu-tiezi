/**
 * Free Board Page Logic - Infinite Scroll Version
 */
...
function setupInfiniteScroll() {
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting && freeState.hasMore && !freeState.isLoading && !freeState.isAppending) {
        loadMoreFreePosts();
      }
    });
...
async function loadMoreFreePosts() {
  if (!freeState.hasMore || freeState.isAppending) return;

  freeState.isAppending = true;
  showInfiniteLoading();
  ...
  try {
    ...
    freeState.hasMore = freeState.currentPage < freeState.totalPages;

    if (newPosts.length > 0) {
      appendPosts(newPosts);
      freeState.posts.push(...newPosts);
    } else {
      freeState.hasMore = false;
    }
  } catch (error) {
    console.error('Failed to load more posts:', error);
  } finally {
    freeState.isAppending = false;
    hideInfiniteLoading();
    if (!freeState.hasMore) showInfiniteEnd();
  }
}
...
function appendPosts(newPosts) {
  ...
  const startIndex = freeState.posts.length;
  newPosts.forEach((post, i) => {
    const item = document.createElement('div');
    item.className = 'masonry-item';
    item.appendChild(renderFreePostCard(post, startIndex + i));
